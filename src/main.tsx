...
async function initMsw(): Promise<void> {
  ...
  try {
    ...
    console.warn('[recipe-spa] MSW не запустился — интерфейс работает, API может быть недоступен:', err);
  }
}

/** Сначала UI (иначе белый экран при зависании MSW на GitHub Pages), затем мок-API в фоне. */
renderApp();
void initMsw();
