...
export function createHandlers() {
  ...
  http.get(apiPath('/recipes'), ({ request }) => {
    const url = new URL(request.url);
    return HttpResponse.json(filterRecipes(url.searchParams));
  }),

  http.get(apiPath('/recipes/latest'), ({ request }) => {
    const url = new URL(request.url);
  ...
  http.get(apiPath('/recipes/:id'), ({ params }) => {
  ...
  http.post(apiPath('/recipes'), async ({ request }) => {
    const userId = readUserIdFromRequest(request.headers.get('Authorization'));
    const user = userId ? getUserById(userId) : undefined;
    if (!user) return unauthorized();
  ...
  http.patch(apiPath('/recipes/:id'), async ({ params, request }) => {
    const userId = readUserIdFromRequest(request.headers.get('Authorization'));
  ...
  http.delete(apiPath('/recipes/:id'), ({ params, request }) => {
    const userId = readUserIdFromRequest(request.headers.get('Authorization'));
    if (!userId) return unauthorized();
    const idx = recipes.findIndex((r) => r.id === params.id);
    if (idx === -1) {
  ...
  http.post(apiPath('/favorites/:recipeId'), ({ params, request }) => {
    const userId = readUserIdFromRequest(request.headers.get('Authorization'));
    if (!userId) return unauthorized();
  ...
  http.delete(apiPath('/favorites/:recipeId'), ({ params, request }) => {
    const userId = readUserIdFromRequest(request.headers.get('Authorization'));
    if (!userId) return unauthorized();
